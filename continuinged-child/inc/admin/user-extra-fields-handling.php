...
add_action('show_user_profile', 'custom_extra_info_fields');
add_action('edit_user_profile', 'custom_extra_info_fields');

function custom_extra_info_fields($user) {
    $email_me = get_user_meta($user->ID, 'email_me', true);
    $email_me_checked = ($email_me == 1) ? 'checked' : '';
    ?>
    ...
    <h3><?php _e('Notification Preferences', 'textdomain'); ?></h3>
    <table class="form-table">
        <tr>
            <th><label for="email_me"><?php _e('Email Notifications', 'textdomain'); ?></label></th>
            <td>
                <label for="email_me">
                    <input type="checkbox" name="email_me" id="email_me" value="1" <?php echo $email_me_checked; ?> />
                    <strong>Receive notifications about new courses</strong><br>
                    <span class="description">
                        Check this box if you want to receive email updates whenever a new course is published.
                    </span>
                </label>
            </td>
        </tr>
...
function save_extra_info($user_id) {
    ...
    // Save email_me preference (luôn lưu 1 hoặc 0)
    $email_me = isset($_POST['email_me']) && $_POST['email_me'] == '1' ? 1 : 0;
    update_user_meta($user_id, 'email_me', $email_me);
}
